...
metadata_all = as.data.frame(read.delim("SupplementaryData1.txt"), header = T, fill = T)
...
#Only display Sublineages with 10 samples or more to avoid too many categories in the legend
...
ggplot(bangladesh, aes(x = Year, y = Freq, fill = Sublineage)) + 
...
ggplot(northindia, aes(x = Year, y = Freq, fill = Sublineage)) + 
...
ggplot(kolkata, aes(x = Year, y = Freq, fill = Sublineage)) + 
...
ggplot(world_cropped) + 
  ...
  geom_text(data = bd1bd2, aes(label = floor(numeric.date), x = (X.from+X.to)/2, y =(Y.from+Y.to)/2))
...
metagenomics = read.table("SupplementaryData3.txt", header = T)
...
ggplot(ridge[which(ridge$Sublineage %in% names(which(table(ridge$Sublineage)> 5))),], aes(x = DecimalDate, y = Sublineage, point_color = Location, fill =Location, color = Location)) + 
...
ggplot(annotation[which(annotation$start > 235585 & annotation$end < 241810 & annotation$seqnames == "AE003853"),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
...
ggplot(annotation[which(annotation$locus_tag %in% paste0("VC0", 489:502)),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
...
ggplot(annotation[which(annotation$locus_tag %in% paste0("VC0", 257:259)),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() + 
...
ggplot(annotation[which(annotation$locus_tag %in% paste0("VCA0", 453:460)),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
...
#acfC
ggplot(annotation[which(annotation$locus_tag %in% paste0("VC0", 840:842)),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
...
#rtxA
ggplot(annotation[which(annotation$locus_tag %in% paste0("VC", 1450:1452)),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
...
ggplot(annotation[which(annotation$start > 518264 & annotation$end < 522264 & annotation$seqnames == "AE003853"),],  aes(xmin = start, xmax = end, fill = locus_tag, y= "", forward = forward)) + geom_gene_arrow() +
  theme_genes() +
  geom_gene_label(align = "left", aes(label = locus_tag)) + geom_feature(y = "", x = 520264) + 
...
a + scale_fill_manual(values = sublin_col[which(sublin_col$Sublineages %in% colnames(df_a)),"col"])

b + scale_fill_manual(values = sublin_col[which(sublin_col$Sublineages %in% colnames(df_b)),"col"])

c + scale_fill_manual(values = sublin_col[which(sublin_col$Sublineages %in% colnames(df_c)),"col"])


arrangeGrob(c+ theme(legend.position = "none",panel.border = element_rect(colour = "white", fill=NA, linewidth=5), title = element_text(size = 12)) + ggtitle("a. Khulna (2015-2018)"),
...
tree_metadata = read.delim("SupplementaryData4.txt", row.names = 1)
...
ggplot(transitions[grep("BD1", transitions$Sample),], aes(y = Parent_Division, x = Division, fill = Freq)) + facet_wrap(~Sample, ncol = 5) + geom_tile() + scale_fill_gradientn(colors = c(brewer.pal(9, "RdPu"), "white"))
ggplot(transitions[grep("BD2", transitions$Sample),], aes(y = Parent_Division, x = Division, fill = Freq)) + facet_wrap(~Sample, ncol = 5) + geom_tile() + scale_fill_gradientn(colors = c(brewer.pal(9, "RdPu"), "white"))

ggplot(as.data.frame(table(subsample[which(subsample$Division != subsample$Parent_Division),c("SampleNumber","Parent_Division", "Lineage")])), aes(x = Parent_Division, y= Freq)) + geom_boxplot() + geom_jitter(width = 0.2) + facet_wrap(~Lineage) + theme_bw() + labs(x = "Source", y = "Number of inter-regional transmission events") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
